gvector.PRWSF = gvector.Layer.extend({
        ...
        if (data && parseInt(data.total_rows)) {
            ...
            for (var i = 0; i < data.rows.length; i++) {
                ...
                if (!onMap || !this.options.uniqueField) {
                    ...
                    var vector_or_vectors = this._geoJsonGeometryToGoogle(data.rows[i].geometry, this._getFeatureVectorOptions(data.rows[i]));
